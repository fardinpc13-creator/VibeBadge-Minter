...
function OwnedBadge({ tokenId, accent, seed }: { tokenId: number; accent: string; seed: number }) {
  ...
  return (
    <div className="badge-card group relative rounded-lg overflow-hidden"
      style={{ border: `1px solid ${accent}40`, background: `linear-gradient(160deg, ${accent}08 0%, #0d0d1a 100%)`, boxShadow: `0 0 20px ${accent}20` }}>
      <div className="p-3">
        ...
        <div className="flex items-center justify-between">
          <p className="font-orbitron text-xs font-bold" style={{ color: accent }}>#{tokenId}</p>
          {uri && (
            <a href={uri as string} target="_blank" rel="noopener noreferrer"
