...
const MAX_B64_KB = 80; // warn above this
...
export function CustomMintTab() {
  ...
  const [imgSrc, setImgSrc]   = useState("");   // final URI sent to contract
  ...
  // Sync imgSrc with mode
  useEffect(() => {
    if (imgMode === "url") setImgSrc(urlInput.trim());
  }, [imgMode, urlInput]);
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (sizeKB > 500) {
      setFileWarn("File too large (max 500KB). Use a URL instead.");
      return;
    }
    if (sizeKB > MAX_B64_KB) {
      setFileWarn(`Large image (${Math.round(sizeKB)}KB) = higher gas cost. Consider resizing.`);
    }
  ...
  const valid  = name.trim().length > 0 && imgSrc.length > 0;
  ...
  function handleMint() {
    ...
    writeContract({
      ...
      args: [address, name.trim(), desc.trim(), imgSrc],
    });
  }
  ...
  const preview = imgSrc || (imgMode === "url" ? urlInput : "");
    ...
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <div className="gradient-border">
        <div className="relative z-10 p-6 sm:p-8 rounded-xl holo-shimmer">
            ...
            <div className="mb-6 flex justify-center">
              <div className="relative w-36 h-36 rounded-lg overflow-hidden corner-cut"
                ...
                <div className="absolute inset-0 opacity-20"
                  style={{ background: "linear-gradient(135deg, #ff00a8, transparent)" }}/>
                {name && (
          ...
          {/* Image mode toggle */}
          <div className="flex mb-4 rounded-sm overflow-hidden"
            style={{ border: "1px solid #ff00a830" }}>
            {(["upload", "url"] as ImgMode[]).map(m => (
              <button key={m} onClick={() => { setImgMode(m); setImgSrc(""); setFileWarn(""); }}
          ...
          {imgMode === "upload" ? (
            <div className="mb-4">
              <div
                className="relative w-full py-8 flex flex-col items-center justify-center cursor-pointer rounded-sm transition-all"
              ...
              >
                ...
                {imgSrc && (
                  <p className="font-mono text-[9px] text-neon-green mt-2">✓ Image loaded</p>
                )}
              </div>
              {fileWarn && (
                <p className="font-mono text-[10px] text-yellow-400 mt-2">{fileWarn}</p>
              )}
            </div>
          ) : (
            <div className="mb-4">
              <label className="font-mono text-[10px] tracking-widest text-neon-magenta block mb-1">
                IMAGE URL *
              </label>
              <input
                value={urlInput}
                onChange={e => { setUrlInput(e.target.value); setImgSrc(e.target.value.trim()); }}
              ...
              <p className="font-mono text-[9px] text-white/20 mt-1">
                Tip: upload to imgur.com → right-click image → copy image address
              </p>
            </div>
          )}
          ...
          <div className="mb-4">
            ...
            <input value={name} onChange={e => setName(e.target.value.slice(0, 32))}
              ...
              style={{ border: "1px solid #00f5ff30", borderRadius: "4px",
                boxShadow: name ? "0 0 8px #00f5ff20" : "none" }}/>
          </div>
          ...
          {!isConnected ? (
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="rounded-sm p-3" style={{ background: "#ff000010", border: "1px solid #ff000030" }}>
                  <p className="font-mono text-[10px] text-red-400 leading-relaxed">
                    ❌ {writeError.message?.includes("user rejected")
                      ...
                      : writeError.message?.includes("Max supply")
                      ? "Max supply reached — all 1000 minted!"
                      : writeError.message?.slice(0, 150) ?? "TX failed. Check wallet & try again."}
                  </p>
                </div>
              )}
              ...
              <p className="font-mono text-[9px] text-white/15 text-center">
                Gas paid in USDC · No mint price · Fully on-chain
              </p>
            </div>
          )}
        </div>
