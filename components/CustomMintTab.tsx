...
const MINT_PRICE = parseEther("0.01"); // 0.01 USDC/ETH as contract requires
...
export function CustomMintTab() {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (sizeKB > 80)   setFileWarn(`Large image (${Math.round(sizeKB)}KB) = higher gas. Consider resizing.`);
    ...
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          ...
          { l: "MINT PRICE",  v: "0.01",                                a: "#ffee00", unit: "USDC" },
          ...
          <div key={s.l} className="text-center p-3 rounded-sm"
            ...
            <p className="font-orbitron text-xl font-black" style={{ color: s.a }}>
              {s.v}{s.unit ? <span className="text-xs"> {s.unit}</span> : ""}
            </p>
        ...
        <div className="relative z-10 p-6 sm:p-8 rounded-xl holo-shimmer">
          ...
          <div className="flex mb-4 rounded-sm overflow-hidden" style={{ border: "1px solid #ff00a830" }}>
            {(["upload", "url"] as ImgMode[]).map(m => (
              <button key={m} onClick={() => { setImgMode(m); setImgSrc(""); setFileWarn(""); if (fileRef.current) fileRef.current.value = ""; }}
          ...
          {imgMode === "upload" ? (
            <div className="mb-4">
              <div className="relative w-full py-8 flex flex-col items-center justify-center cursor-pointer rounded-sm transition-all"
            ...
            <div className="mb-4">
              ...
              <p className="font-mono text-[9px] text-white/20 mt-1">
                Tip: upload to imgur.com → right-click → copy image address
              </p>
            </div>
          )}
          ...
          <div className="mb-4">
            ...
            <input value={name} onChange={e => setName(e.target.value.slice(0, 32))}
              ...
              className="w-full bg-dark-700 font-rajdhani text-sm text-white placeholder-white/20 px-3 py-2.5 outline-none"
              style={{ border: "1px solid #00f5ff30", borderRadius: "4px", boxShadow: name ? "0 0 8px #00f5ff20" : "none" }}/>
          </div>
          ...
          {!isConnected ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              {step === "error" && writeError && (
                <div className="rounded-sm p-3" style={{ background: "#ff000010", border: "1px solid #ff000030" }}>
                  <p className="font-mono text-[10px] text-red-400 leading-relaxed">
                    ❌ {writeError.message?.includes("user rejected") ? "Transaction rejected in wallet."
                      : writeError.message?.includes("Pay") ? "Mint price not met — need 0.01 USDC."
                      : writeError.message?.slice(0, 150) ?? "TX failed."}
                  </p>
                </div>
              )}
              {step === "pending" && txHash && (
              ...
              <p className="font-mono text-[9px] text-white/15 text-center">
                Mint price: 0.01 USDC · Paid in USDC gas
              </p>
            </div>
          )}
        </div>
